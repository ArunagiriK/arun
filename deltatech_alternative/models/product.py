...
class product_template(models.Model):
    ...
    alternative_code = fields.Char(string='Alternative Barcode', index=True, compute='_compute_alternative_code')
    alternative_ids = fields.One2many('product.alternative', 'product_tmpl_id', string='Alternatives')
    alternative_name = fields.Char('Alternative name',compute='_compute_alternative_code')
    ...
    def _compute_alternative_code(self):
        ...
        for cod in self.alternative_ids:
            if cod.name and not cod.hide:
                codes += [cod.name]
            if cod.product_name_exist:
               names += [cod.product_name_exist]
...
class product_product(models.Model):
    ...
    alternative_code = fields.Char(string='Alternative Barcode')
    ...
    def name_search(self, name='',args=None, operator='ilike', limit=100):
        ...
        if name and len(name)> 2 :
            alternative_ids = self.env['product.alternative'].search(['|',('name', 'ilike', name),('product_name_exist','ilike',name)], limit=10)
            # ids = []
...
class product_alternative(models.Model):
    ...
    product_name_exist = fields.Char(string='Name',index=True)
