...
class EmployeeFormInherit(models.Model):
    ...
    def start_grounding(self):
        for em in self:
            if em.contracts_count < 1:
                raise ValidationError(_("You cannot start grounding the employee %s without a valid contract") %(em.name))
            em.state = 'grounding'
            em.stages_history.sudo().create({'start_date': date.today(),
                                           'employee_id': self.id,
                                           'state': 'grounding'})
    ...
    def start_employment(self):
        for em in self:
            if em.contracts_count < 1:
                raise ValidationError(_("You cannot start the employment for employee named %s without a valid contract") %(em.name))
            em.set_as_employee()
            em.contract_id.write({'state': 'open'})

    @api.multi
    def view_set_as_employee(self):
        self.ensure_one()
        if self.contracts_count < 1:
            raise ValidationError(_("You cannot start grounding the employee %s without a valid contract") %(self.name))
        ctx = dict(self.env.context or {})
        res_id = self.id     
        [employee_stage_action] = self.env.ref('employee_stages.wizard_employee_act').read()
        ctx['employee_id'] = res_id
        employee_stage_action['context'] = ctx
        return employee_stage_action
