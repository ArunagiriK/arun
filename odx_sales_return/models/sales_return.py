...
class OdxSalesRetun(models.Model):
    _name = 'odx.sale.return'
    ...
    order_line = fields.One2many('odx.sale.return.line', 'order_id', string='Order Lines',
    ...
    picking_ids = fields.One2many('stock.picking', 'sale_return_id', string='Pickings')
    ...
    def action_create_credit_note(self):
        ...
        for line in self.order_line:
            if line.return_configuration_type_id:
                if line.return_configuration_type_id.refund:
                    if line.product_id.property_account_income_id:
                        account = line.product_id.categ_id.property_account_income_id.id
                    else:
                    ...
                    invoice_vals_list.append((0, 0, invoice_line_vals))
        if not invoice_vals_list:
            raise UserError(_('There is no refundable  product'))
        else:
    ...
    def action_confirm(self):
        ...
        if self.order_line:
            for configuration_type in self.env['odx.return.configuration'].search([]):
                filtered_lines = self.order_line.filtered(lambda x: x.return_configuration_type_id.id == configuration_type.id)
                if filtered_lines:
                    move_vals_list = []
                    for line in filtered_lines:
                        move_vals = {'product_id':line.product_id.id,
                                     ...
                                     'location_dest_id':line.return_configuration_type_id.location_id.id}
    ...
    def _get_invoiced(self):
        ...
        line_invoice_status_all = [(d['order_id'][0], d['invoice_status']) for d in self.env['odx.sale.return.line'].read_group([('order_id', 'in', self.ids), ('product_id', '!=', deposit_product_id.id)], ['order_id', 'invoice_status'], ['order_id', 'invoice_status'], lazy=False)]
        for order in self:
    ...
    def get_empty_list_help(self, help):
        ...
        return super(OdxSalesRetun, self).get_empty_list_help(help)
    ...
    name = fields.Char(string='Order Reference', required=True, copy=False, index=True, default=lambda self: _('New'), readonly=True,)
    ...
    order_line = fields.One2many('odx.sale.return.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True, auto_join=True)
    ...
    company_id = fields.Many2one('res.company', 'Company', default=lambda self: self.env['res.company']._company_default_get('odx.sale.return'))
    ...
    def _compute_access_url(self):
        super(OdxSalesRetun, self)._compute_access_url()
        for order in self:
    ...
    def unlink(self):
        ...
        return super(OdxSalesRetun, self).unlink()
    ...
    def _track_subtype(self, init_values):
        ...
        return super(OdxSalesRetun, self)._track_subtype(init_values)
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            if 'company_id' in vals:
                vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code('odx.sale.return') or _('New')
            else:
                vals['name'] = self.env['ir.sequence'].next_by_code('odx.sale.return') or _('New')
        ...
        result = super(OdxSalesRetun, self).create(vals)
        return result
    ...
    def _write(self, values):
        ...
        if self.env.context.get('mail_activity_automation_skip'):
            return super(OdxSalesRetun, self)._write(values)
    ...
    def copy_data(self, default=None):
        ...
        return super(OdxSalesRetun, self).copy_data(default)
    ...
    def name_get(self):
        if self._context.get('sale_show_partner_name'):
            ...
            return res
        return super(OdxSalesRetun, self).name_get()
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        return super(OdxSalesRetun, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
    ...
    def message_post(self, **kwargs):
        ...
        return super(OdxSalesRetun, self.with_context(mail_post_autofollow=True)).message_post(**kwargs)
    ...
    def _notify_get_groups(self, message, groups):
        ...
        groups = super(OdxSalesRetun, self)._notify_get_groups(message, groups)
    ...
    def _get_share_url(self, redirect=False, signup_partner=False, pid=None):
        ...
        return super(OdxSalesRetun, self)._get_share_url(redirect, signup_partner, pid)
...
class OdxSalesRetunLine(models.Model):
    _name = 'odx.sale.return.line'

    order_id = fields.Many2one('odx.sale.return', string='Order Return Reference', required=True, ondelete='cascade',
                               index=True, copy=False)
    return_configuration_type_id = fields.Many2one('odx.return.configuration', string="Return Configuration",
                                                   required=True)

    qty_delivered_method = fields.Selection([
        ('manual', 'Manual'),
        ('analytic', 'Analytic From Expenses')
    ], string="Method to update delivered qty", store=True,
        readonly=True,
    ...
    def write(self, values):
        ...
        result = super(OdxSalesRetunLine, self).write(values)
        return result
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        return super(OdxSalesRetunLine, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
    ...
    def unlink(self):
        ...
        return super(OdxSalesRetunLine, self).unlink()
...
class OdxSalesRetunConfiguration(models.Model):
    _name = 'odx.return.configuration'

    name = fields.Char('Name', required=True)
    refund = fields.Boolean('Refund', required=True)
    location_id = fields.Many2one('stock.location', string="Return Location", required=True)


class StockPicking(models.Model):
    _inherit = 'stock.picking'

    sale_return_id = fields.Many2one('odx.sale.return', string="Sale Return", readonly=True)
