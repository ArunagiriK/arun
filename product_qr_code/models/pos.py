...
class product_product(models.Model):
    ...
    @api.constrains('qr_code', 'company_id')
    def _check_barcode(self):
        if self.barcode and self.search_count(
                [('qr_code', '=', self.qr_code), ('company_id', '=', self.company_id.id)]) > 1:
            raise ValidationError(_('A Qrcode can only be assigned to one product per company'))
        return True
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        if name:
            ...
            if operator in positive_operators:
                ...
                if not product_ids:
                    product_ids = self._search([('qr_code', operator, name)] + args, limit=limit, access_rights_uid=name_get_uid)
                if not product_ids:
