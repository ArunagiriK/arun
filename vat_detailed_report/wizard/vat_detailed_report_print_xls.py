...
class AccountVatDetailedReportWizard(models.TransientModel):
    ...
    def write_lines(self, sheet, row, tax_detail_data, data_format, total_format):        
    ...
    def get_tax_detail_data(self, move_types=[]):
        query_type = """  """
        args_list = ()
        if move_types:
    ...
    def generate_excel(self):
        ...
        tax_receivable_data = self.get_tax_detail_data(move_types=['receivable'])
        tax_receivable_refund_data = self.get_tax_detail_data(move_types=['receivable_refund'])
        tax_payable_data = self.get_tax_detail_data(move_types=['payable'])
        tax_payable_refund_data = self.get_tax_detail_data(move_types=['payable_refund'])
        row = 12
        ...
        row += 1
        row = self.write_lines(sheet, row, tax_receivable_data, format5, format7)   
        sheet.merge_range(row, 0, row, 7, 'Taxes Paid On Credit Notes', format2)     
        row += 1
        row = self.write_lines(sheet, row, tax_receivable_refund_data, format5, format7)  
        sheet.merge_range(row, 0, row, 7, 'Taxes Paid', format2)    
        row += 1
        row = self.write_lines(sheet, row, tax_payable_data, format5, format7)       
        sheet.merge_range(row, 0, row, 7, 'Taxes Received On Debit Notes', format2)      
        row += 1
        row = self.write_lines(sheet, row, tax_payable_refund_data, format5, format7)
